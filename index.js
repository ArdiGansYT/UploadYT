...
CFonts.say('\'YouTube:\' Drawl ArdiGansYT', {
  font: 'console',
  align: 'center',
  gradient: ['red', 'magenta']
})
...
/******** OWNER NUMBER**********/
const ownerNumber = ["6285282996146@s.whatsapp.net","6283102650464@s.whatsapp.net"] //ganti nomor lu
/************************************/
  ...
  if (text) conn.sendFile(m.chat, global.API('xteam', '/attp', { file: '', text }), 'attp.webp', '', m, false, { asSticker: true })
				...
				case 'bcgc':
                   ...
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group Patrick :\n\n\`\`\``
					no = 0
